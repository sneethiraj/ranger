...
public class RangerSSOAuthenticationFilter implements Filter {
    ...
    private final SSOAuthenticationProperties jwtProperties;
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        } else if (!ssoEnabled && ((HttpServletRequest) servletRequest).getRequestURI().contains(RestUtil.LOCAL_LOGIN_URL) && !isAuthenticated() && (isWebUserAgent(userAgent) || isBrowserAgent(userAgent))) {
    ...
    private Authentication getGrantedAuthority(Authentication authentication) {
        if (authentication != null && authentication.isAuthenticated()) {
            ...
            UsernamePasswordAuthenticationToken result       = new UsernamePasswordAuthenticationToken(userDetails, authentication.getCredentials(), grantedAuths);
